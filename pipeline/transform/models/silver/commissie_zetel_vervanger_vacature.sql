{{
    config(
        ...
        deletion_relation='bronze.commissie_zetel_vervanger_vacature',
...
WITH latest AS (
    SELECT
        id,
        commissie_zetel__ref AS commissie_zetel_id,
    ...
    FROM {{ source('bronze', 'commissie_zetel_vervanger_vacature') }}
