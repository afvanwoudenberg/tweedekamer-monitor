{{
    config(
        unique_key=['activiteit_id', 'voortgezet_vanuit_id'],
        incremental_strategy='merge_with_deletes',
        deletion_relation='none',
        on_schema_change='fail',
        contract={'enforced': true}
    )
}}

WITH links AS (
    SELECT DISTINCT
        parent.id AS activiteit_id,
        ref AS voortgezet_vanuit_id
    FROM {{ source('bronze', 'activiteit__voortgezet_vanuit') }} AS link
    INNER JOIN {{ source('bronze', 'activiteit') }} AS parent
        ON link._dlt_parent_id = parent._dlt_id
    WHERE link.ref IS NOT NULL
),

incoming AS (
    SELECT links.*
    FROM links
    INNER JOIN {{ ref('activiteit') }} AS activity
        ON links.activiteit_id = activity.id
    INNER JOIN {{ ref('activiteit') }} AS continued
        ON links.voortgezet_vanuit_id = continued.id
)

SELECT * FROM incoming